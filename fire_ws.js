 ...
 var datos = [
     ...
     { topic: "anaquel/ws/vent", value: "" },
     { topic: "anaquel/ws/warmlight", value: "" },
     { topic: "anaquel/ws/riego", value: "" },
     { topic: "anaquel/ws/temperature", value: "" },
     { topic: "anaquel/ws/humidity", value: "" },
 ...
 client.on("connect", function() { //this library automatically reconnects on errors
     try {
         ...
         client.publish("anaquel/ws/automatic", 'on');
     } catch (ex) {
         console.log(ex);
     }
 });
 ...
 function control_ws() {
     ...
     if (timer(timer_par.ws.led, moment().hours(), moment().minutes())) client.publish(datos[0].topic, 'off');
     else client.publish(datos[0].topic, 'on'); //console.log("led")

     if (timer(timer_par.ws.warm, moment().hours(), moment().minutes())) client.publish(datos[2].topic, 'off');
     else {
